...
sum(case when activity_type='send' then time_spent else 0 end) as send_time_sum,
sum(case when activity_type='open' then time_spent else 0 end) as open_time_sum
FROM activities A
...
select age_bucket,
round((sum(send_time_sum)/(sum(send_time_sum)+sum(open_time_sum)))*100.0,2) as send_perc,
round((sum(open_time_sum)/(sum(send_time_sum)+sum(open_time_sum)))*100.0,2) as open_perc
from time_calc
group by age_bucket;
